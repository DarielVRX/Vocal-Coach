...
const TimelineDraw = {
    ...
    _calcUmbralGap(puntos) {
        if (!puntos || puntos.length < 4) return 0.2;
        const gaps = [];
    ...
    _drawTimeAxis(W, H) {
        ...
        // Intervalo adaptativo según zoom
    ...
    _drawSegmentosRef(W, H) {
        // Referencia como puntos usando el mismo sistema de segmentos
        const ctx    = this.ctx, labelW = 36;
        const tAhora = this._tAhora();
